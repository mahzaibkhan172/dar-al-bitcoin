  ...
  return (
    <section ref={ref} className="section-padding bg-gradient-to-b from-white to-gray-50">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {modules.map((module, index) => (
            <motion.div
              ...
              className="card p-6 border-2 border-transparent hover:border-bitcoin-400 transition-all duration-300"
            >
        ...
        >
          <h3 className="text-2xl font-outfit font-bold text-navy-900 text-center mb-8">
            Ce Qui Est Inclus
          </h3>
            ...
            {features.map((feature, index) => (
              ...
              >
                <div className="w-12 h-12 bg-bitcoin-500/10 rounded-full flex items-center justify-center">
                  <feature.icon className="text-bitcoin-500" size={24} />
                </div>
                <p className="text-navy-900 text-sm">{feature.text}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <Link to="/formation">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="px-8 py-4 rounded-xl font-semibold border-2 border-navy-900 text-navy-900 hover:bg-navy-900 hover:text-white transition-all duration-300 text-lg"
            >
